...
(function () {
	...
	$.fn.mapsed = function (options) {
		...
		function applyTemplate(tmpl, model, $ctx) {
			...
			tmpl = replaceAll("{cidade}", model.cidade, tmpl);
			tmpl = replaceAll("{CEP}", model.CEP, tmpl);
			tmpl = replaceAll("{bairro}", model.bairro, tmpl);
			tmpl = replaceAll("{numero}", model.numero, tmpl);
			tmpl = replaceAll("{telefone}", model.telefone, tmpl);
			tmpl = replaceAll("{estado}", model.estado, tmpl);
		...
		function getViewTemplate() {
			...
			var html =
			  ...
			  + "<input class='cidade d-none' type='text' value='{cidade}' />"
			  + "<input class='CEP d-none' type='text' value='{CEP}' />"
			  + "<input class='bairro d-none' type='text' value='{bairro}' />"
			  + "<input class='numero d-none' type='text' value='{numero}' />"
			  + "<input class='telefone d-none' type='text' value='{telefone}' />"
			  + "<input class='estado d-none ' type='text' value='{estado}' />"
			  + "</td>"
			  + "</tr>"
			  + "<tr>"
			  + "<td class='mapsed-left'>"
			  + "<address>"
			  + "<div class='mapsed-street d-none'>{STREET}</div>"
		...
		function getEditTemplate() {
			var html =
				...
				+ "<li>"
				+ "<label>Name"
				+ "<input class='mapsed-name' type='text' value='{NAME}' />"
				+ "</label>"
				+ "</li>"
				+ "<li>"
				+ "<label>Street"
				+ "<input class='mapsed-street d-none' type='text' placeholder='e.g. 3 Hemington place' value='{STREET}' />"
				+ "</label>"
				+ "</li>"
				+ "<li>"
				+ "<label>Town"
		...
		function getViewModel($vw) {
			...
			if ($vw.hasClass("mapsed-view")) {
				model.nome = $vw.find(".mapsed-name").attr("title");
				model.street = $vw.find(".mapsed-street").html();
				model.cidade = $vw.find(".cidade").val();
				model.CEP = $vw.find(".CEP").val();
				model.bairro = $vw.find(".bairro").val();
				model.numero = $vw.find(".numero").val();
				model.telefone = $vw.find(".telefone").val();
				model.estado = $vw.find(".estado").val();
				...
				// editor view
				model.nome = $vw.find(".mapsed-name").val();
				model.cidade = $vw.find(".cidade").val();
				model.CEP = $vw.find(".CEP").val();
				model.bairro = $vw.find(".bairro").val();
				model.numero = $vw.find(".numero").val();
